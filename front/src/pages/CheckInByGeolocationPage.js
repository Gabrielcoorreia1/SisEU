// src/pages/CheckInByGeolocationPage.js
...
const extractCleanName = (raw) => {
    if (!raw || typeof raw !== "string") return "Participante";

    const nomeMatch = raw.match(/Nome\s*=\s*([^,}]+)/i);
    const sobrenomeMatch = raw.match(/Sobrenome\s*=\s*([^}]+)/i);

    const nome = nomeMatch ? nomeMatch[1].trim() : "";
    const sobrenome = sobrenomeMatch ? sobrenomeMatch[1].trim() : "";

    const full = `${nome} ${sobrenome}`.trim();

    if (full.includes("Ouvinte")) return nome || "Participante";
...
const CheckInByGeolocationPage = () => {
    ...
    const [localFlowMode, setLocalFlowMode] = useState(localStorage.getItem('flowMode') || 'CHECKIN');
    const isCheckout = localFlowMode === 'CHECKOUT';
    const eventPin = localStorage.getItem('currentPin');

    const currentUser = JSON.parse(localStorage.getItem('currentUser') || '{}');

    const nomeUsuario = extractCleanName(currentUser.nome);
    ...
    useEffect(() => {
        if (!eventPin && !isCheckout && subStep !== GEOLOC_STEPS.SUCESSO) {
            alert("Erro de segurança: Código de Check-in não encontrado. Reinicie o processo.");
            localStorage.removeItem('flowMode');
            localStorage.removeItem('currentPin');
            navigate('/dashboard');
        }
    }, [eventPin, isCheckout, subStep, navigate]);


    const handleConfirmarDados = async () => {
        if (!isCheckout && (!eventPin || !coords)) {
            setError("PIN ou localização não encontrados. Tente novamente.");
            return;
        }
        if (isCheckout && !coords) { 
        ...
        try {
            if (isCheckout) {
                await CheckinService.registrarCheckout(coords.latitude, coords.longitude);
                
                localStorage.removeItem('flowMode');
                localStorage.removeItem('currentPin'); 
                setLocalFlowMode('CHECKIN'); 
            } else {
            ...
            const apiErrorString = err.message || JSON.stringify(err);
            if (!isCheckout && apiErrorString.includes("Você já registrou o Check-in")) {
                localStorage.setItem('flowMode', 'CHECKOUT');
                setLocalFlowMode('CHECKOUT'); 
                setSubStep(GEOLOC_STEPS.GEOLOC_CONFIRM); 
                setLoading(false);
                alert("Presença Aberta Detectada. Por favor, confirme para Check-out.");
                return;
            }
            setError(apiErrorString || `Falha na validação.`);
        } finally {
            setLoading(false);
        }
    }
        ...
        if (!isPermitted || gpsError) {
            return (
                <div style={{ color: 'var(--theme-text)', textAlign: 'center' }}>
                    ...
                    <p style={{ color: 'red' }}>Não foi possível obter sua localização no momento. Verifique as permissões e guarde.</p>
                </div>
            );
        }
        if (!coords) {
                ...
                return (
                    <div style={{ textAlign: 'center' }}>
                        ...
                        <Button 
                            ...
                            text={`Confirmar Localização para ${currentActionText}`}
