...
const AdmPage = () => {
    ...
    const handleGenerateReports = async () => {
        try {
            const csvBlob = await RelatorioService.generateReport(); 
            
            const url = URL.createObjectURL(csvBlob);
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `Relatorio_Presenca_${new Date().toISOString().slice(0, 10)}.csv`);
        ...
        try {
            ...
            if (confirm) {
                ...
                if (pinAtivo) {
                    ...
                    alert("SUCESSO PARCIAL: O PIN não foi gerado. Verifique a API.");
                }
            }
